...
### 4.6. Flash Writes & Audio DMA Panics (The `adc_continuous` Bug)
During implementation of the Captive Portal, the act of saving WiFi credentials to NVS caused consistent Guru Meditation errors (`Cache disabled but cached memory region accessed`). This occurred because the `adc_continuous` DMA audio polling driver is not IRAM-safe by default, meaning its interrupts panic the system if they fire while a flash write (which disables the system cache) is occurring.

*   **Lesson:** Rather than abandoning the high-performance DMA driver for a CPU-intensive busy-wait `adc_oneshot` implementation, it is far better to explicitly pause the audio driver strictly around the discrete operations that write to flash (e.g. `nvs_commit`, `UserFS.open(FILE_WRITE)`). This keeps CPU usage low without panicking the ESP32.
