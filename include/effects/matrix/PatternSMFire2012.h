...
class PatternSMFire2012 : public EffectWithId<PatternSMFire2012>
{
  private:
    ...
    static constexpr auto palette_arr = std::to_array<const TProgmemRGBPalette16*>({
        ...
        for (int x = 0; x < MATRIX_WIDTH; x++)
        {
            ...
            for (int y = 0; y < MATRIX_HEIGHT; y++)
                nblend(g()->leds[XY(x, MATRIX_HEIGHT - 1 - y)],
                       ColorFromPalette(*curPalette, ((noise3d[x][y] * 0.7f) + (noise3d[wrapX(x + 1)][y] * 0.3f))),
                       fireSmoothing);
        }
    }
};
