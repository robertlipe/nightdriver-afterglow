...
class PatternSMWisp : public EffectWithId<PatternSMWisp>
{
  private:
    ...
    static constexpr int CENTER_X_MINOR =
    ...
    static constexpr int WISP_NUM_LAYERS = 1; // в кометах используется 1 слой, но для огня 2018 нужно 2
    static constexpr int WISP_NUM_LAYERSMAX = 2;
    ...
    NoiseLayer noiseLayers[WISP_NUM_LAYERSMAX];

    uint8_t noise3d[WISP_NUM_LAYERSMAX][MATRIX_WIDTH][MATRIX_HEIGHT]; // двухслойная маска или хранилище свойств в
                                                                 // размер всей матрицы
  ...
  public:
    ...
    void MoveFractionalNoiseX(int amplitude = 1, float shift = 0)
    {
    ...
    void FillNoise(int layer)
    {
        ...
        {
            int32_t ioffset = noiseLayers[layer].scale_x * (i - CENTER_X_MINOR);
            for (int j = 0; j < MATRIX_HEIGHT; j++)
            {
                int32_t joffset = noiseLayers[layer].scale_y * (j - CENTER_Y_MINOR);
                int data = inoise16(noiseLayers[layer].noise_x + ioffset, noiseLayers[layer].noise_y + joffset, noiseLayers[layer].noise_z) >> 8;
    ...
    void Draw() override
    {
        ...
        // Noise
        noiseLayers[0].noise_x += 1500;
        noiseLayers[0].noise_y += 1500;
        noiseLayers[0].noise_z += 1500;
        noiseLayers[0].scale_x = 8000;
        noiseLayers[0].scale_y = 8000;
        FillNoise(0);
